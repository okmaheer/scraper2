<?php
	...
	foreach ( $chapter['storage'][ $in_use ]['page'] as $page => $link ) {
		...
		// Extract the matched part if available
			...
			// Assuming $cleaned_src contains the URL of the image
			$image_format = pathinfo($cleaned_src, PATHINFO_EXTENSION);
			$image_url = esc_url($cleaned_src);

			if ($image_format !== 'webp') {
				$image_url = '/proxy.php?url=' . urlencode($image_url);
			}
			...
			<?php echo $image_url; ?>" class="<?php echo esc_attr( $lazyload ); ?>">
